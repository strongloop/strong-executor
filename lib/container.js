...
Container.prototype.setEnv = function(env, callback) {
  ...
  this._env = env;
  this.restart({soft: false}, function(err) {
    self.debug('Restart with new env: %s', err ? err.message : 'ok');
    if (callback)
      callback(err);
  });
};
...
Container.prototype._run = function(callback) {
  ...
  this.debug('run %s %s', run, args.join(' '));
...
Container.prototype.stop = function(options, callback) {
  ...
  var proc = options.proc || self._proc;
  if (!proc) {
  ...
  if (options.soft) {
    timeoutHandle = setTimeout(function() {
      ...
      self.stop({proc: proc}, function(err, reason) {
